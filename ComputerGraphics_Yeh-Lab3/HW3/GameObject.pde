...
void debugDraw() {
    Matrix4 MVP = main_camera.Matrix().mult(localToWorld()); // 計算模型到屏幕的變換矩陣
  
    for (int i = 0; i < mesh.triangles.size(); i++) {
        Triangle triangle = mesh.triangles.get(i);
        Vector3[] worldVerts = new Vector3[3];
        for (int j = 0; j < 3; j++) {
            worldVerts[j] = localToWorld().mult(triangle.verts[j].getVector4(1.0)).homogenized();
        }
        ...
        // 計算視線向量
        Vector3 viewVector = cam_position.sub(worldVerts[0]).unit_vector();

        // 判斷是否需要剔除（背面剔除）
        if (Vector3.dot(normal, viewVector) > 0) {
            continue; // 如果法向量與視線夾角超過 90 度，跳過繪製
        }
